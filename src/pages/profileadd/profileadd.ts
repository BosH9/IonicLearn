...
export class ProfileaddPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public alrtCtrl: AlertController,
    ...
    this.getUserData(navParams.get("rowid"));
  }
  ...
  save() {
    ...
    if (this.regForm.valid) {
      ...
      this.sqlite.create({
        ...
        db.executeSql('INSERT INTO usersample VALUES(NULL,?,?,?,?,?,?,?)',[this.user.name,this.user.address,this.user.gender,this.user.city,this.user.image,this.user.dob,dt])
          .then(res => {
            console.log(res);
